...
class Upscale:
    ...
    def run(self) -> None:
        ...
        if not self.input.is_file():
            self.log.error(f'File "{self.input}" is a not a file.')
            sys.exit(1)
        ...
        with Progress(
            ...
            task_upscaling = progress.add_task("Upscaling", total=1)

            img_path = self.input

            img_input_path_rel = img_path.relative_to(self.input)
            img_output_path_rel = self.output

            if len(model_chain) == 1:
                self.log.info(
                    f'Processing {str(idx).zfill(len(str(1)))}: "{img_input_path_rel}"'
                )
            ...
            if len(model_chain) > 1:
                task_model_chain = progress.add_task(
                    f'{str(idx).zfill(len(str(1)))} - "{img_input_path_rel}"',
...
def main(
        ...
        input: Path = typer.Option(Path("input"), "--input", "-i", help="Input File"),
        output: Path = typer.Option(Path("output"), "--output", "-o", help="Output File"),
