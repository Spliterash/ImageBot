...
def kmeans(input_img, k, i_val):
    ...
    return centers[i_val].astype(int)
...
while True:
    ...
    if biggest.size != 0:
        ...
        kmean = kmeans(input_img=imgWarpGray, k=8, i_val=2)
        _, anotherThres = cv2.threshold(imgWarpGray, kmean, 255,
                                        cv2.THRESH_BINARY)

        # Image Array for Display
        imageArray = (
            [img, imgWarpGray, imgAdaptiveThre, anotherThres],
        )
    ...
    # LABELS FOR DISPLAY
    lables = [
        ["Original", "Gray", "Threshold", "MyThres"],
        ["Original", "Gray", "Threshold", "MyThres"],
    ]
    ...
    if cv2.waitKey(1) & 0xFF == ord('s'):
        count = len(glob.glob("Scanned/*"))
        cv2.imwrite("Scanned/myImage" + str(count + 1) + ".png", stackedImage)
